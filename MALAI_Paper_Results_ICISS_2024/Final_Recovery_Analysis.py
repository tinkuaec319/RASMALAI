...
min_no_mod_80, min_samples_80 = process_file("Minimum_samples_required_for_80_Percent_Recovery.txt")
min_no_mod_90, min_samples_90 = process_file("Minimum_samples_required_for_90_Percent_Recovery.txt")
min_no_mod_100, min_samples_100 = process_file("Minimum_samples_required_for_100_Percent_Recovery.txt")
...
ws.append([
    "Hamming Weight",
    "For 80% Recovery Minimum NoMod%", "For 80% Recovery Minimum #Samples Required",
    "For 90% Recovery Minimum NoMod%", "For 90% Recovery Minimum #Samples Required",
...
for hw in hamming_weights:
    ...
    # 80% recovery
    if hw in min_no_mod_80 and min_no_mod_80[hw] is not None and min_samples_80[hw] is not None:
        row.append(min_no_mod_80[hw])
        row.append(min_samples_80[hw])
    else:
        row.extend([-1, -1])

    # 90% recovery
    if hw in min_no_mod_90 and min_no_mod_90[hw] is not None and min_samples_90[hw] is not None:
        row.append(min_no_mod_90[hw])
        row.append(min_samples_90[hw])
    else:
        row.extend([-1, -1])
...
output_filename = "Final_Recovery_Analysis.xlsx"
...
transposed_filename = "Final_Transposed_Recovery_Analysis.xlsx"
