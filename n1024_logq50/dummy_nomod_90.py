import os
import numpy as np
from sklearn.linear_model import LinearRegression, OrthogonalMatchingPursuit
import time

# Start time
start_time = time.time()

# ================= PARAMETERS =================
start_hwt = 3
end_hwt = 40
nomod_percentage = 0.90
Q = 607817174438671
half_value = Q // 2
# ==============================================

# Initialize Hamming weight recovery tracker
# hwt_recovered = {hwt: False for hwt in range(start_hwt, end_hwt + 1)}

hwt_recovered = {hwt: True for hwt in range(start_hwt, end_hwt + 1)}
for hwt in [3,4,5,9]:
    if hwt in hwt_recovered:
        hwt_recovered[hwt] = False

# Load data (FIXED WITH F-STRINGS)
secret_data = np.load(f'ternary_secrets_h{start_hwt}_{end_hwt}/secret.npy', allow_pickle=True)
train_b = np.load(f'ternary_secrets_h{start_hwt}_{end_hwt}/train_b.npy', allow_pickle=True)
modified_A = np.load('modified_train_A.npy', mmap_mode='r')


# Models and sample sizes
models = [LinearRegression(), OrthogonalMatchingPursuit()]
num_samples_list = [50000]


# Recover secret function for ternary secret
def recover_secret(model, filtered_A, filtered_b):
    model.fit(filtered_A, filtered_b)
    recovered_s = model.coef_ if hasattr(model, 'coef_') else model.feature_importances_

    # Compute the thresholds for ternary recovery
    min_value = np.min(recovered_s)
    mean_value = np.mean(recovered_s)
    max_value = np.max(recovered_s)

    threshold1 = (min_value + mean_value) / 2
    threshold2 = (max_value + mean_value) / 2

    # Convert the recovered values to ternary values
    recovered_ternary_s = np.where(recovered_s < threshold1, -1, np.where(recovered_s > threshold2, 1, 0))
    return recovered_ternary_s

# Main processing loop
for num_samples in num_samples_list:
    output_dir = f"NoMod_{nomod_percentage*100:.0f}%_results_for_h{start_hwt}_{end_hwt}_just_try"  # Dynamic percentage
    os.makedirs(output_dir, exist_ok=True)
    output_filename = os.path.join(
        output_dir, 
        f"Secret_recovery_results_for_{num_samples}_samples_with_Nomod_{nomod_percentage*100:.0f}%_try.txt"
    )
    
    with open(output_filename, 'w') as output_file:
        b_col, true_s_col = 0, 0
        
        for hamming_weight in range(start_hwt, end_hwt + 1):
            if hwt_recovered[hamming_weight]:
                output_file.write(f"\n****************** Hamming Weight = {hamming_weight} ******************\n")
                for _ in range(10):
                    output_file.write("2\t\t\tpreviously recovered\n")
                output_file.write(f"******************Ending of hamming weight {hamming_weight} ******************\n\n\n\n")
                b_col += 10
                true_s_col += 10
                continue
                
            output_file.write(f"\n****************** Hamming Weight = {hamming_weight} ******************\n")
            total_success = 0
            
            for iteration in range(10):
                true_s = secret_data[:, true_s_col]
                b = train_b[:, b_col]
                modified_b = np.where(b > half_value, b - Q, b)
                
                # Optimized index calculation
                abs_diff = np.abs(np.dot(modified_A, true_s) - modified_b)
                mask = abs_diff < half_value
                no_mod_indices = np.where(mask)[0]
                mod_indices = np.where(~mask)[0]
                
                # NoMod-controlled sample selection
                num_no_mod = int(nomod_percentage * num_samples)  # Using parameter
                num_mod = num_samples - num_no_mod
                
                if len(no_mod_indices) < num_no_mod or len(mod_indices) < num_mod:
                    output_file.write("Insufficient data\n")
                    b_col += 1
                    true_s_col += 1
                    continue
                
                selected_indices = np.concatenate([
                    np.random.choice(no_mod_indices, num_no_mod, False),
                    np.random.choice(mod_indices, num_mod, False)
                ])
                np.random.shuffle(selected_indices)
                
                # Model processing
                iter_success = 0
                start_iter = time.time()
                
                for model in models:
                    recovered = recover_secret(model, 
                                             modified_A[selected_indices], 
                                             modified_b[selected_indices])
                    if np.array_equal(recovered, true_s):
                        iter_success += 1
                        
                total_success += iter_success
                elapsed = time.time() - start_iter
                output_file.write(f"{iter_success}\t\t\t{elapsed:.2f} seconds\n")
                
                b_col += 1
                true_s_col += 1

            if total_success == 20:
                hwt_recovered[hamming_weight] = True
                
            output_file.write(f"******************Ending of hamming weight {hamming_weight} ******************\n\n\n\n")

print("Optimized processing complete")
# End time
end_time = time.time()
total_time = end_time - start_time
print(f"\n\nTotal time taken to execute the script: {total_time:.2f} seconds\n\n")

