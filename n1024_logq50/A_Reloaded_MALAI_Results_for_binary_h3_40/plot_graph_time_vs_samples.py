...
def process_file_for_time(filename, hamming_weights):
    ...
    for line in lines:
        ...
        if line.startswith("****************** Hamming Weight ="):
            ...
            try:
                if "seconds" in line:
                    time_str = line.split("seconds")[0].split()[-1]
...
plt.rc('axes', titlesize=label_font_size)
plt.rc('axes', labelsize=label_font_size)
plt.rc('xtick', labelsize=font_size)
plt.rc('ytick', labelsize=font_size)

x_values = [n // 10000 for n in num_samples_list]  # x-axis: in units of 10,000
plt.plot(x_values, average_times, linestyle='-', color='blue')

plt.xlabel("Number of Samples (x 10,000)")
plt.ylabel("Average Recovery Time (Seconds)")
plt.title("Average Secret Recovery Time vs Number of Samples (N=1024)")
plt.xticks(range(0, 101, 10))
plt.grid(True)
plt.tight_layout()
plt.savefig("Recovery_Time_vs_Samples.png")
plt.show()
