...
num_samples_list = [10000, 20000, 50000, 100000, 200000]
...
def analyze_and_plot():
    ...
    for nomod in no_mod_percentages:
        folder_name = f"NoMod_{nomod}%_results_for_h{start_hwt}_{end_hwt}_ternary"
        ...
        for samples in num_samples_list:
            file_name = f"Secret_recovery_results_for_{samples}_samples_with_Nomod_{nomod}%_ternary.txt"
    ...
    plt.legend(fontsize=legend_font_size, loc='lower right', framealpha=0.9)
    plt.grid(True, linestyle='--', alpha=0.6)
    
    plt.xticks(np.arange(0, 50, 5), fontsize=font_size)
    plt.yticks(np.arange(0, 101, 10), fontsize=font_size)
    plt.xlim(1, max(num_samples_list)//10000 + 5)
    plt.ylim(-5, 105)

    plt.tight_layout()
    plt.savefig("Ternary_Final_till_1M_NoMod_80.png", dpi=300, bbox_inches='tight')
    plt.show()
